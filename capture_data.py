...
class Combinations:
    ...
    END_KEYS = [Key.esc, Key.f4]  # key combination to finish listening
    NEXT_WORD_KEYS = [Key.space, ':', ",", "/", '"']  # key of next words
    NEW_CONTEXT_KEYS = [Key.down, Key.up]  # , keyboard.Key.dot},
    SENTENCE_END_KEYS = [Key.enter, ".", ";", '?', '!']
    NUMPAD_NUMBERS_KEYS = [Key.n_zero, Key.n_one, Key.n_two,
                           Key.n_three, Key.n_four,
                           Key.n_five, Key.n_six, Key.n_seven,
                           Key.n_eight, Key.n_nine]
...
class RealTimeKeyListener:
    ...
    def __init__(self):
        ...
        self.__pressed_keys = []
        self.keyboard_listener = Listener(on_press=self.__on_press)
        self.mouse_listener = Listener(on_click=self.__on_click)
    ...
    def __on_press(self, key):
        ...
        elif key == Key.delete and self.__sentence and self.__position != 0:
            self.__delete_chars()

        elif key == Key.backspace and self.__sentence:
            self.__backspace_chars()

        elif key == Key.left and abs(self.__position) <= len(self.__sentence):
            self.__position -= 1

        elif key == Key.right and self.__position < 0:
            self.__position += 1

        elif (hasattr(key, 'char') and key.char is not None and len(key.char) < 2) or (
                key in Combinations.NUMPAD_NUMBERS_KEYS or key == Key.space):
    ...
    def __record_non_printable_digraphs_keys(self, click):
        if (isinstance(click,
                       Key) and click not in Combinations.NUMPAD_NUMBERS_KEYS and self.__pressed_keys is not None) or (isinstance(self.__previous_key,
                                                                                                                                  Key) and click not in Combinations.NUMPAD_NUMBERS_KEYS):
            self.__non_printable_digraphs.append([str(self.__previous_key), str(click)])

    def __count_no_printable_keys(self, click):
        if isinstance(click, Key) and click not in Combinations.NUMPAD_NUMBERS_KEYS:
    ...
    def __insert_key(self, key):
        ...
        if hasattr(key, 'char') and key.char is not None and len(key.char) < 2:
            char = key.char
        elif key in Combinations.NUMPAD_NUMBERS_KEYS or key == Key.space:
